...
function loadLiteFunctions({ socket: lite, data }) {
  ...
  const waitReply = async (text) => {
    await waitReact();
    return await reply(` espere! ${text || waitMessage}`);
  };

  const warningReply = async (text) => {
    await warningReact();
    return await reply(` Alerta! ${text}`);
  };

  const errorReply = async (text) => {
    await errorReact();
    return await reply(` Error! ${text}`);
  };
  ...
  const infoFromSticker = async (info) => {
    ...
    if (isImage) {
        ...
        async (error) => {
          if (error) {
            ...
            throw new DangerError(
              "No se pudo convertir su imagen!\n\n",
      ...
      if (!haveSecondsRule) {
        ...
        throw new DangerError(
          `su video supero los ${sizeInSeconds} segundos!
      ...
      exec(
        ...
        async (error) => {
          if (error) {
            deleteTempFile(inputPath);
            throw new DangerError(
              "Error al covertir su video a sticker!\n\n",
              JSON.stringify(error, null, 2)
            );
          }
...
async function getProfileImageData(userJid, lite) {
  ...
  try {
    ...
    profileImage = path.resolve(ASSETS_DIR, "images", "user.png");
